...
def generate_dc_hour_topic(model_type, model_name):
    config.llm_model_type = model_type
    ...
    report, report_file_path = report_generator.generate_dc_topic_report(markdown_file_path)  # 生成并获取报告内容及文件路径
...
with gr.Blocks(title="GitHubSentinel") as demo:
    ...
    # 创建 Dogecoin 热点话题 Tab (新增)
    with gr.Tab("Dogecoin 热点话题"):
        gr.Markdown("## Dogecoin 热点话题")  # 添加小标题
        ...
        # 创建按钮来生成 Dogecoin 热点话题报告
        button = gr.Button("生成 Dogecoin 热点话题")
        ...
        # 将按钮点击事件与导出函数绑定
        button.click(generate_dc_hour_topic, inputs=[model_type, model_name], outputs=[markdown_output, file_output])
